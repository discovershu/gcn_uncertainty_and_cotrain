...
def load_voc_data(seed, test_ratio, img_name, scribble):
    # read images
    ...
    labels = np.zeros([RGB_ground_truth.shape[0] * RGB_ground_truth.shape[1], 2], dtype=np.float)
    labels_index = 0
    train_index_scribble = []
    for i in range(RGB_ground_truth.shape[0]):
        for j in range(RGB_ground_truth.shape[1]):
            if RGB_ground_truth[i][j] == 0 or RGB_ground_truth[i][j] == 255: #or RGB_ground_truth[i][j] == 255
    ...
    for j in range(labels.shape[1]):
        for i in range(labels.shape[0]):
            if labels[i][j] == 1.0:
                ...
                if j == 1:
                    train_1.append(i)
    train_indices_0 = random_state.choice(train_0, int(test_ratio * len(train_0)), replace=False)
    train_indices_1 = random_state.choice(train_1, int(test_ratio * len(train_1)), replace=False)
